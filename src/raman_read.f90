  ...
  USE  raman_mod, ONLY : prefix, outdir, fil_dvec, fil_elph, &
       ...
       k, q, wk, wq, eigv, eq, &
       matele_elph, dvec, elaser, temp, efermi, &
       kb, pi, im, ev2ry, ry2cm
  !
  IMPLICIT NONE
  ...
  namelist / inputraman / &
       ...
       sorb, circular_pol, nonpol, plot_matele_opt, plot_matele_elph, plot_raman_k, &
       gamma, gamma_raman, rs_start, rs_end, nrs, temp, efermi, &
  ...
  plot_raman_k = .FALSE.
  gamma = 0.01d0          ! Broadening for photon lifetime in eV
  gamma_raman = 0.0001d0  ! Broadening for Raman spectra width in eV
  rs_start = 0.0
  rs_end = 800.0
  nrs = 1000
  temp = 300.0d0   ! Temperature in K
  efermi = 0.0d0   ! Fermi energy in eV
  elaser1 = 2.33d0 !or 532 nm
  elaser2 = 0.0d0  ! Laser energy in eV
  elaser3 = 0.0d0
  elaser4 = 0.0d0
  elaser5 = 0.0d0
  elaser6 = 0.0d0
  ...
  ENDIF
  !
  ! modify the unit to atomic unit eV --> Ry, cm^-1 --> Ry
